...
The bonus part of the project is to apply the skills learned to create an automatic response to SSH brute force attempts, effectively stopping them by blocking the attacking IP.

#### 8.1 Set Up Ubuntu Machine
We will start by creating a new droplet with the following steps:
- Ubuntu System
- Create a droplet using the lowest specifications (this will be targeted by brute force attackers).
- Use a very strong password to protect this machine, as it will attract attention from attackers.
- Allow ALL inbound traffic: This will allow SSH brute force attempts to target the machine, providing data for testing.

![image](https://github.com/user-attachments/assets/375de682-c158-4fb3-b78c-c66f192bc572)

After accessing VM remember to update and upgrade all packages.
#### 8.2 Install Wazuh Agent on Ubuntu
Configure the firewall on your Wazuh Manager to allow traffic from the Ubuntu agent

![image](https://github.com/user-attachments/assets/61f5896e-3d95-473d-84e9-cc6dd7019d37)

Log in to the Wazuh Manager web interface, go to the agent deployment section, and deploy a new agent. Select the installation option for Ubuntu and follow the steps provided in the Wazuh interface.

![image](https://github.com/user-attachments/assets/dbf6bda5-a622-494a-b934-c9ff1cee3225)

#### 8.3 Forward All Level 5 Alerts to Shuffle
Now that the Wazuh Agent is configured, it's time to integrate it with Shuffle to automatically forward all level 5 alerts (such as SSH brute force attempts). To use the Wazuh API, we need to first authenticate and retrieve a JSON Web Token.

Drag and drop the HTTP App (called http in Shuffle) onto the workflow editor. I will name it "Get API" In the settings for this app, set the Action to "Curl." Add Curl Command to Authenticate with Wazuh API
```
curl -u <user>:<password> -k -X GET "https://localhost:55000/security/user/authenticate?raw=true"
```
To utilize Wazuh API capabilites this command will extract JSON Web Token from Wazuh.

For Shuffle to connect to the Wazuh API, you need to ensure that Wazuh Manager can accept traffic through port 55000 from Shuffle. Update your firewall rules on the Wazuh Manager to allow inbound traffic on port 55000 from the Shuffle server.

To set up the connection with the Wazuh API, you’ll need the Wazuh API user credentials, which are located inside the Wazuh Manager desktop.
```
tar -axf wazuh-install-files
```
Inside the extracted files, find the wazuh-passwords file.This file contains the username and password required for Wazuh API access.

Additionaly let's configure active response. 
We are interested in command `<firewall-drop>`. This will allow us to modify iptables configurations of our ubuntu agent, essentialy to drop traffic coming from malicious ip.

Open `ossec.conf` file and find the commented <active-response> block:
```
  <!--
  <active-response>
  ...
  -->
```
Modify this block to enable firewall-drop for level 5 alerts:
```
  <active-response>
...
```
This means we will execute `firewall-drop` command on `local` machine
**Remember to restart wazuh manager!**

One thing to mind, to check response name we must headover to /var/ossec/bin and execute `./agent_control -L`.

![image](https://github.com/user-attachments/assets/d881844a-2c6c-4560-a1f1-41f19585ae14)

You'll see that the response name will be something like firewall-drop0, where the 0 comes from the timeout="no" configuration. If you set a timeout (e.g., 50 seconds), you would see firewall-drop50.

When you execute `./agent_control` you will se available options for active response.
For example `./agent_control -b 8.8.8.8 -f firewall-drop0 -u 002` will execute response for ip address 8.8.8.8 with command firewall-drop0 for agent id 002.

I will showcase that by pinging 8.8.8.8 (google.com) on Ubuntu Agent.

![image](https://github.com/user-attachments/assets/a020e2b4-22ae-4f1f-9f2b-915b18cfec16)

![image](https://github.com/user-attachments/assets/78f79629-11e4-454f-93dd-e03928629174)

You can check if active response was successful on the agent. Check `/var/ossec/logs/active-responses.log`.

**Return to the Shuffle**
For now Workflow will look like this:

![image](https://github.com/user-attachments/assets/b9322c59-8360-4e4c-937a-74b1d74c0eda)

For the Wazuh App.
- We want action set as "Run command"
- Apikey is coming from "Get API" we set up earlier
- URL of course will be ip address of Wazuh Manager
- Set Agent List with Agent ID under execution arguments
- "Wait for complete" to "True"
- Set argument as 8.8.8.8
- Add command "firewall-drop0"
